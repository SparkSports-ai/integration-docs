...
Money is sent in minor units as integers. `isRoundFinished` closes a round. We deduplicate on `casinoSessionId` plus our transaction id, and retries never double-charge or double-credit.
